...
debitmugc <- ggmugtotc +
  annotation_custom(
    ggplotGrob(ggmugc),
    xmin= as.Date("2019-07-08"), xmax = as.Date("2019-11-30"), ymin=500, ymax=2100
  )
```
...
debitlauc <- gglautotc +
  annotation_custom(
    ggplotGrob(gglauc),
    xmin= as.Date("2019-06-01"), xmax = as.Date("2019-11-25"), ymin=20, ymax=65
  )
```

```{r figdebit}
ggarrange(debitmugc, debitlauc, nrow = 2, labels = c("(a)", "(b)"),
          # common.legend = TRUE,
          # legend="bottom",
...
mod=lmer(particles_m3_cor ~ code_station*crue + (1|replicate), data=lmp_samplingA_crue)
qqnorm(resid(mod))
hist(resid(mod))
Anova(mod, Type="III")
...
mod=lmer(particles_m3_cor ~ code_station*log(debit_m3_s) + (1|replicate), data=lmp_samplingA_crue)
qqnorm(resid(mod))
hist(resid(mod))
Anova(mod)
...
gglau19 = ggplot(lmp_crue_lau, 
              aes(log(debit_m3_s+1), conc_nb_plastic_m3))+ # LAU / Debit
  geom_point(size=3)+
  #facet_wrap(.~ code_station, scales = "free")+
...
Microplastic concentration (MP.m^-3^) during sampling events in the sites MUG and GSG in (a) flood A and (b) flood B.

```{r figconcab, echo=FALSE, message=FALSE, warning=FALSE}
ggarrange(ggarrange(crueconca, ncol = 1, labels = c("(a)", "")),   # First row with Flood A plots
          ggarrange(crueconcb, ncol = 1, labels = c("(b)", "", "")), # Second row with Flood B plots
          nrow = 2,
          common.legend = TRUE,
          labels = "",
          font.label = list(size = 12)) 
...
Relationship between microplastic concentration (MP.m^-3^) and river discharge (log-transformed, m^3^.s^-1^) in the sites MUG (displayed in blue) and GSG (in green) for (a) flood A and (b) flood B.

```{r figconcabdebit, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r figfamdflood, echo=FALSE, warning=FALSE, message=FALSE}
ggarrange(ggarrange(FA1flood, FA1floodgsg, ncol = 2, labels = c("(a)", "(b)")),  
...
ggarrange(ggarrange(FA1_lau, FA2_lau, ncol = 2, labels = c("", "")),  
          ggarrange(FA3_lau, FA4_lau, ncol = 2, labels = c("", "")), 
          nrow = 2, 
          labels = "LAU",
          font.label = list(size = 12))
```
...
```{r figfamddebit, message=FALSE, warning=FALSE, include=FALSE}
...
```{r figpcaflood, include=FALSE, warning=FALSE, message=FALSE}
ggarrange(ggarrange(PC1flood, PC1floodgsg, ncol = 2, labels = c("(a)", "(b)")),  
